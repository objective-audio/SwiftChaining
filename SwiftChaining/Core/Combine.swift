...
extension Chain {
    private func combine0<Out1, Sender1>(_ chain1: Chain<Out1, Sender1>) -> Chain<(Out, Out1), Sender> {
    ...
    private func cache<T0, T1>() -> Chain<Out, Sender> where Out == (T0?, T1?) {
    ...
    private func complete<T0, T1>() -> Chain<(T0, T1), Sender> where Out == (T0?, T1?) {
